...
/*   Updated: 2022/11/29 00:23:18 by hidhmmou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ft_count_words(char const *str, char charset, int *ac)
{
	int	i;
...
char	**ft_split(char const *s, char c, int *ac)
{
	...
	if (!s)
		return (NULL);
	i = 0;
	splited = malloc(sizeof(char *) * ft_count_words(s, c, ac));
	if (!splited)
		return (NULL);
	while (*s)
	{
